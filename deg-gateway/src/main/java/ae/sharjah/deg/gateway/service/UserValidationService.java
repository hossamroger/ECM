...
public class UserValidationService {
    ...
    public UserValidationService(RestTemplate restTemplate, DegProperties props) {
        this.restTemplate = restTemplate;
        this.props = props;
    }

    /**
     * @return {@code true} if the caller is authorized (or validation is disabled),
     *         {@code false} if the validateUser service reported 401.
     */
    public boolean isAuthorized(String dstoken) {
        DegProperties.Validation cfg = props.getValidation();
        if (!cfg.isEnabled()) {
            return true;
        }
        if (!StringUtils.hasText(cfg.getUrl())) {
            log.warn("Validation is enabled but deg.validation.url is not set; skipping validation.");
            return true;
        }
